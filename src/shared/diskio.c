...
DSTATUS disk_initialize(BYTE pdrv) {
	...
	set_detail_code(0);
	outportw(IO_NILE_SPI_CNT, NILE_SPI_DEV_NONE | NILE_SPI_390KHZ);
	nile_tf_cs_high();
...
card_init_complete_hc:
	...
	outportw(IO_NILE_SPI_CNT, NILE_SPI_DEV_NONE | NILE_SPI_25MHZ);
	if (!nile_spi_rx(1, NILE_SPI_MODE_READ))
		return false;
	return 0;
}
...
DRESULT disk_write (BYTE pdrv, const BYTE __far* buff, LBA_t sector, UINT count) {
	...
	while (count) {
		...
		if (!nile_spi_rx_copy(resp, 2, NILE_SPI_MODE_WAIT_READ)) {
