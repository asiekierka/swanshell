...
// -1 = full redraw
static inline void ui_dialog_draw_buttons(ui_dialog_config_t *config, int16_t selected) {
    uint16_t button_x[UI_DIALOG_MAX_BUTTON_COUNT];
    uint16_t button_w[UI_DIALOG_MAX_BUTTON_COUNT];
    ...
    for (int i = 0; i < UI_DIALOG_MAX_BUTTON_COUNT; i++) {
...
void ui_dialog_draw(ui_dialog_config_t *config) {
    ...
    if (config->buttons[0]) {
        ...
        ui_dialog_draw_buttons(config, -1);
...
int16_t ui_dialog_action(ui_dialog_config_t *config, uint8_t selected_button) {
    if (config->buttons[0]) {
        ...
        for (int i = 1; i < UI_DIALOG_MAX_BUTTON_COUNT; i++, button_count++) {
            if (!config->buttons[i]) break;
        }

        ui_dialog_draw_buttons(config, 0);
        while (true) {
            wait_for_vblank();
            input_update();
            ...
            if (prev_selected_button != selected_button) {
                ui_dialog_draw_buttons(config, selected_button);
            }
