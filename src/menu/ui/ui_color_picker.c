...
static const uint8_t __far colorbar_mono_gradient[] = {
  0xff, 0xff, 0xbf, 0xbb, 0xbb, 0xbb, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa,
  0xaa, 0xaa, 0x22, 0x22, 0x22, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
  0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x77, 0x77, 0x77, 0x57,
  0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x15, 0x11, 0x11, 0x11, 0x00,
  0xff, 0xef, 0xee, 0xee, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa,
  0x8a, 0x88, 0x88, 0x08, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
  0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xdd, 0xdd, 0x5d, 0x55, 0x55,
  0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x45, 0x44, 0x44, 0x00, 0x00, 0x00,
};
...
static void draw_colorbar_mono(uint8_t y, uint8_t start_palette, uint16_t rgb, uint8_t rgb_shift, uint16_t lang_key, bool full_redraw, bool masked) {
    ...
    if (full_redraw) {
        ws_screen_modify_tiles(bitmap_screen2,
            ...
            COLORBAR_X >> 3, y >> 3,
            COLORBAR_ENTRY_WIDTH * 2, COLORBAR_ENTRY_HEIGHT >> 3);
    }
...
void ui_color_picker(uint16_t *rgb) {
    ...
    bool mono = !ws_system_is_color_active();
    ...
    while (true) {
        ...
        if (input_pressed & WS_KEY_X1) {
            if (selector_offset > 0)
                ...
                selector_offset = mono ? 3 : 2;
        }
        if (input_pressed & WS_KEY_X3) {
            if (selector_offset < (mono ? 3 : 2))
                selector_offset++;
            else
                selector_offset = 0;
        }
